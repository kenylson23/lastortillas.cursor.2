...
var supabasePoolerUrl = `postgresql://postgres.nuoblhgwtxyrafbyxjkw:Kenylson%4023@aws-0-us-east-1.pooler.supabase.com:5432/postgres`;
var databaseUrl = process.env.DATABASE_URL || supabasePoolerUrl;
...
var DatabaseStorage = class {
  ...
  async createTable(insertTable) {
    await this.ensureInitialized();
    console.log(`\u{1F50D} Verificando duplica\xE7\xE3o para mesa ${insertTable.tableNumber} no local ${insertTable.locationId}`);
    ...
    console.log(`\u{1F50D} Mesa existente encontrada:`, existingTable);
    if (existingTable) {
      throw new Error(`J\xE1 existe uma mesa n\xFAmero ${insertTable.tableNumber} no local ${insertTable.locationId}`);
    }
  ...
  async updateTable(id, updates) {
    await this.ensureInitialized();
    if (updates.tableNumber !== void 0 || updates.locationId !== void 0) {
      const currentTable = await this.getTable(id);
      if (!currentTable) {
        throw new Error("Mesa n\xE3o encontrada");
      }
      const newNumber = updates.tableNumber !== void 0 ? updates.tableNumber : currentTable.tableNumber;
  ...
  try {
    ...
    if (method === "PUT" && query.id) {
      ...
      if (body.status && req.url?.includes("/status")) {
        const updatedTable2 = await storage.updateTableStatus(id, body.status);
        return res.status(200).json(updatedTable2);
      }
