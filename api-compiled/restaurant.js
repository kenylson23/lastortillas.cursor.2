    ...
    try {
        ...
        if (req.url?.includes('/orders')) {
            if (method === 'GET' && query.id) {
                ...
                const order = await storage_serverless_1.storage.getOrder(id);
                if (!order) {
            ...
            if (method === 'GET') {
                const orders = await storage_serverless_1.storage.getAllOrders();
                return res.status(200).json(orders);
            }
            if (method === 'POST') {
                const validatedData = orderSchema.parse(body);
                const { items, ...orderData } = validatedData;
                const newOrder = await storage_serverless_1.storage.createOrder(orderData, items);
                return res.status(201).json(newOrder);
            }
            if (method === 'PUT' && query.id) {
                const id = parseInt(query.id);
                const { status } = body;
                const updatedOrder = await storage_serverless_1.storage.updateOrderStatus(id, status);
                return res.status(200).json(updatedOrder);
            }
            if (method === 'DELETE' && query.id) {
                const id = parseInt(query.id);
                await storage_serverless_1.storage.deleteOrder(id);
        ...
        if (req.url?.includes('/reservations')) {
            if (method === 'GET') {
                const reservations = await storage_serverless_1.storage.getAllReservations();
                return res.status(200).json(reservations);
            }
            if (method === 'POST') {
                const validatedData = reservationSchema.parse(body);
                const newReservation = await storage_serverless_1.storage.createReservation(validatedData);
        ...
        if (req.url?.includes('/contacts')) {
            if (method === 'POST') {
                ...
                const newContact = await storage_serverless_1.storage.createContact(validatedData);
        ...
        if (req.url?.includes('/availability')) {
            if (method === 'GET') {
                ...
                const available = await storage_serverless_1.storage.checkAvailability(date, time);
