...
async function handler(req, res) {
    ...
    try {
        ...
        if (method === 'GET' && !query.id) {
            const tables = await storage_1.storage.getAllTables();
        ...
        if (method === 'POST') {
            ...
            const newTable = await storage_1.storage.createTable(validatedData);
        ...
        if (method === 'PUT' && query.id) {
            ...
            if (body.status && req.url?.includes('/status')) {
                const updatedTable = await storage_1.storage.updateTableStatus(id, body.status);
                return res.status(200).json(updatedTable);
            }
            // Regular table update
            const validatedData = tableSchema.partial().parse(body);
            const updatedTable = await storage_1.storage.updateTable(id, validatedData);
            return res.status(200).json(updatedTable);
        }
